...
// New instantiates and returns and Kubernetes discoverer.
func New() (k Kubernetes) {
...
func (k Kubernetes) addFunc(obj interface{}) {
	...
	err = k.renderer.Execute(pod.GetName(), pod.Status.PodIP, k.datastore)
	if err != nil {
...
func (k Kubernetes) updateFunc(oldObj, newObj interface{}) {
	...
	if oldPod.Status.PodIP == "" && newPod.Status.PodIP != "" {
		err := k.renderer.Execute(newPod.GetName(), newPod.Status.PodIP, k.datastore)
		if err != nil {
			log.Printf("Failed to render template: %v", err)
		}
	}
